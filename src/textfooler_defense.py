...
simplifier = Simplifier(threshold=3000, ratio=0.25, syn_num =20, most_freq_num =10)
simp = simplifier.random_freq_v1
...
class NLI_infer_BERT(nn.Module):
    ...
    def text_pred(self, text_data,simlifier, detector, batch_size=32):
        # Switch the model to eval mode.
        self.model.eval()

        # transform text data into indices and create batches
        dataloader0 = self.dataset.transform_text(text_data, simlifier, detector, batch_size=batch_size)
        dataloader1 = self.dataset.transform_text(text_data, simlifier, detector, batch_size=batch_size)
        dataloader2 = self.dataset.transform_text(text_data, simlifier, detector, batch_size=batch_size)

        probs_all_0 = []
        probs_all_1 = []
        probs_all_2 = []
        #         for input_ids, input_mask, segment_ids in tqdm(dataloader, desc="Evaluating"):
        for input_ids, input_mask, segment_ids in dataloader0:
            input_ids = input_ids.cuda()
            input_mask = input_mask.cuda()
            segment_ids = segment_ids.cuda()
            ...
            with torch.no_grad():
                logits = self.model(input_ids, input_mask, segment_ids)
                probs = nn.functional.softmax(logits[0], dim=-1)
                probs_all_0.append(probs)

        for input_ids, input_mask, segment_ids in dataloader1:
            input_ids = input_ids.cuda()
            input_mask = input_mask.cuda()
            segment_ids = segment_ids.cuda()
            ...
            with torch.no_grad():
                logits = self.model(input_ids, input_mask, segment_ids)
                probs = nn.functional.softmax(logits[0], dim=-1)
                probs_all_1.append(probs)

        for input_ids, input_mask, segment_ids in dataloader2:
            input_ids = input_ids.cuda()
            input_mask = input_mask.cuda()
            segment_ids = segment_ids.cuda()
            ...
            with torch.no_grad():
                logits = self.model(input_ids, input_mask, segment_ids)
                probs = nn.functional.softmax(logits[0], dim=-1)
                probs_all_2.append(probs)

        probs_all_sum = [i+j+k for i,j,k in zip(probs_all_0,probs_all_1,probs_all_2)]
        probs_all = [x/3 for x in probs_all_sum]
        #print("probs_all_0:", probs_all_0, "\n", "probs_all:", probs_all)
...
class NLIDataset_BERT(Dataset):
    ...
    def convert_examples_to_features(self, examples, max_seq_length, tokenizer, simplify, detector):
        """Loads a data file into a list of `InputBatch`s."""

        features = []
        for (ex_index, text_a) in enumerate(examples):

            detect_probs = detector([text_a])#.squeeze()

            if 1 == torch.argmax(detect_probs, dim=-1):
            #if 1 == 1:
                random_number = random.randint(0,6)
                if random_number == 0:
                    text_a = simp(' '.join(text_a))
                else:
                    sent = ' '.join(text_a)
                    lab = '0'
                    sam = {}
                    sam['x'] = sent
                    sam['y'] = lab
                    sa_sample = SASample(sam)
                    if random_number == 1:
                        trans_sample = InsertAdv().transform(sa_sample)
                    if random_number == 2:
                        trans_sample = MLMSuggestion().transform(sa_sample)
                    if random_number == 3:
                        trans_sample = BackTrans().transform(sa_sample)
                    if random_number == 4:
                        trans_sample = SwapSynWordNet().transform(sa_sample)
                    if random_number == 5:
                        trans_sample = Tense().transform(sa_sample)
                    if random_number == 6:
                        trans_sample = Contraction().transform(sa_sample)
                    if trans_sample != []:
                        text_a = trans_sample[0].dump()['x']
                    else:
                        text_a = simp(' '.join(text_a))
            else:
                text_a = ' '.join(text_a)
    ...
    def transform_text(self, data, simplify, detector, batch_size=32):
        # transform data into seq of embeddings
        eval_features = self.convert_examples_to_features(data,
                                                          self.max_seq_length, self.tokenizer,simplify, detector)
...
def attack(text_ls, true_label, predictor, detector, stop_words_set, word2idx, idx2word, cos_sim,simplify, sim_predictor=None,
           import_score_threshold=-1., sim_score_threshold=0.5, sim_score_window=15, synonym_num=50,
           batch_size=32):
    # first check the prediction of the original text
    orig_probs = predictor([text_ls],simplify,detector).squeeze()
    ...
    if true_label != orig_label:
        ...
        leave_1_probs = predictor(leave_1_texts, simplify, detector, batch_size=batch_size)
        ...
        for idx, synonyms in synonyms_all:
            ...
            new_probs = predictor(new_texts, simplify, detector, batch_size=batch_size)
            ...
            if np.sum(new_probs_mask) > 0:
                ...
                if new_label_prob_min < orig_prob:
                    ...
                    num_changed += 1
            text_cache = text_prime[:]
        return ' '.join(text_prime), num_changed, orig_label, torch.argmax(predictor([text_prime],simplify, detector)), num_queries
...
def main():
    ...
    parser.add_argument("--detector_path", type=str, help="xxx classifier")

    args = parser.parse_args()
    if args.do_simplify:
        print("do randomization the randomization version is " + args.simplify_version)

        simplify_dict = {'insertadv': InsertAdv().transform,
                         'mlmsuggestion': MLMSuggestion().transform,
                         'swapsynwordnet': SwapSynWordNet().transform,
                         'backtrans': BackTrans().transform,
                         'none': None}
    ...
    print("Model built!")
    detector_model = NLI_infer_BERT_detector(args.detector_path, nclasses=2, max_seq_length=args.max_seq_length)
    ...
    for idx, (text, true_label) in enumerate(tqdm(data)):
        ...
        if args.perturb_ratio > 0.:
            ...
            new_text, num_changed, orig_label, \
            new_label, num_queries = attack(text, true_label, predictor, detector, stop_words_set,
                                            word2idx, idx2word, cos_sim, simplify, sim_predictor=use,
                                            sim_score_threshold=args.sim_score_threshold,
                                            import_score_threshold=args.import_score_threshold,
                                            sim_score_window=args.sim_score_window,
